...
import {initialCards, validationConfig, formAddCard, formEditProfile, listContainerEl, popupOpenButton, addBtn, inputEditProfileName, inputEditProfileAbout, heading, headingDescription} from '../utils/constants.js'
...
const userInfo = new UserInfo({userNameSelector: ".profile__heading", userInfoSelector: ".profile__heading-description"});
const imagePopupHandler = new PopupWithImage({popupSelector: '.popup_image'});
const popupEditProfileForm = new PopupWithForm({popupSelector: '.popup-edit-profile', handleFormSubmit: (formValues) => {
  userInfo.setUserInfo(formValues);
  }
});
const sectionElement = new Section({}, listContainerEl);
const popupAddCardForm = new PopupWithForm({popupSelector: '.popup-add-card', handleFormSubmit: (formValues) => {
  const newCard = createCard(formValues, ".element-template");
  
  sectionElement.addItemPrepend(newCard)
  }
});
